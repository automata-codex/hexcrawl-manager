import { HexFileNotFoundError } from '@skyreach/data';
...
// If you have shared diff/summary printers, swap these with your helpers.
type SummaryRow = {
  hex: string;
  file: string;
  ...
  for (const [hex, intent] of Object.entries(intents)) {
    ...
    if (fileChanged) {
      ...
      if (opts.dryRun) {
        ...
        fs.writeFileSync(tmp, yaml.stringify(nextDoc), 'utf8');
