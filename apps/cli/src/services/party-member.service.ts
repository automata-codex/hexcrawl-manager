...
export function isGuest(member: PartyMember): member is { playerName: string; characterName: string } {
  return typeof member === 'object' && 'playerName' in member && 'characterName' in member;
}
...
export function matchesPartyMember(member: PartyMember, identifier: string): boolean {
  if (isGuest(member)) {
