...
describe("scribe start", () => {
  ...
  it("aborts with error if lock file for next session sequence exists (production mode)", async () => {
    await withTempRepo("scribe-start-lock-conflict", { initGit: false }, async (repo) => {
      ...
      const { exitCode, stderr } = await runScribe(commands, { repo, ensureExit: false, ensureFinalize: false });
      expect(exitCode).toBe(0); // REPL exits normally
      expect(stderr).toMatch(new RegExp(`Lock file exists for session sequence 27 \(.*\)\. Another session may be active\.`, 'i'));
