...
>
> - `weave allocate ap absence` — spend absence credits (this document).
...
* Validate **available absence credits** (derived at runtime).
...
All characters earn absence credits regardless of tier — the rules of record (`character-advancement.md`) state that absent players gain one downtime AP per missed session, with no tier restriction.

**Non-goals:** producing/altering play AP (`session_ap`), or emitting machine-readable output.
...
```bash
# Spend N credits for a character, mapping credits to pillars explicitly
...
* **Pillar sum matches amount.** `combat + exploration + social == amount`; else **fail**.
...
> Mirrors `weave ap status`. Guests never accrue/spend. Tier is irrelevant for absence-credit accrual — every character gets one credit per missed non-downtime session.
...
* `pillars: { combat?: number, exploration?: number, social?: number }`
  (matches CLI distribution; pillar sum must equal `amount`)
...
* Character not found / pillar sum mismatch / insufficient credits → non-zero with specific message.
* Success → exit 0.
...
* Character with pillar split; verifies `absence_spend` in ledger + report audit row.
