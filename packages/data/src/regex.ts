...
export const ROLLOVER_FILE_RE =
  /^rollover_(\d{4})-(spring|summer|autumn|winter)\.jsonl$/i;
...
export const SESSION_FILE_RE =
  /^session[-_](\d{4})([a-z])?_(\d{4}-\d{2}-\d{2})\.jsonl$/i; // TODO eventually we'll only have the hyphen between "session" and the number
