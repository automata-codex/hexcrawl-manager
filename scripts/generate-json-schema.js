...
writeFileSync(classFile, JSON.stringify(zodToJsonSchema(ClassSchema), null, 2));

const encounterEntryFile = new URL(
  '../schemas/encounter.json',
  import.meta.url,
);
writeFileSync(
  encounterEntryFile,
...
const encounterTableFile = new URL(
  '../schemas/encounter-table.json',
  import.meta.url,
);
writeFileSync(
  encounterTableFile,
...
writeFileSync(mapPath, JSON.stringify(zodToJsonSchema(MapPathSchema), null, 2));

const npcFile = new URL('../schemas/npc.json', import.meta.url);
writeFileSync(npcFile, JSON.stringify(zodToJsonSchema(NpcSchema), null, 2));
...
const supplementFile = new URL(
  '../schemas/supplement-list.json',
  import.meta.url,
);
writeFileSync(
  supplementFile,
...
const unusedHiddenSiteFile = new URL(
  '../schemas/unused-hidden-site.json',
  import.meta.url,
);
writeFileSync(
