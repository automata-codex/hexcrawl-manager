  ...
  switch (t.type) {
    ...
    case 'reflection': {
      ...
      if (sig) {
        const params = (sig.parameters ?? [])
          .map(
            (p) =>
              `${p.name}${p.flags?.isOptional ? '?' : ''}: ${renderType(p.type)}`,
          )
          .join(', ');
    ...
    case 'templateLiteral':
      ...
      return (
        ...
        [t.head, ...(t.tail ?? [])]
          .map((chunk, i) => {
            const hole = t.types?.[i]
              ? '${' + renderType(t.types[i]) + '}'
              : '';
            return hole + chunk;
          })
          .join('') +
...
function eachTopLevelExport(entry) {
  ...
  function visit(n, depth = 0) {
    ...
    if (Array.isArray(n.signatures)) {
      n.signatures.forEach((s) => visit(s, depth + 1));
    }
    if (n.type?.declaration) {
      visit(n.type.declaration, depth + 1);
    }
  }
...
function renderNodeLine(n) {
  ...
  switch (n.kind) {
    case KIND.Class:
      code = renderClassHeader(n);
      break;
    case KIND.Enum:
      code = renderEnumSignature(n);
      break;
    case KIND.Function:
      code = renderFunctionSignature(n);
      break;
    case KIND.Interface:
      code = renderInterfaceHeader(n);
      break;
    case KIND.TypeAlias:
      code = renderTypeAliasSignature(n);
      break;
    case KIND.Variable:
      code = renderVariableSignature(n);
      break;
    default:
      return null;
  }
...
function renderEntrySection(entry, byId) {
  ...
  const nodes = (
    ids.length ? ids.map((id) => byId.get(id)) : (entry.children ?? [])
  )
    .filter(Boolean)
